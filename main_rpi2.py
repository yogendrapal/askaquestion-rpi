...
class check_buttons(Thread):
	...
	def counter_label(self):
		...
		def count():
				global counter
				counter -= 1
				self.widget.config(text="Please wait for "+str(counter)+"  secs")
				if counter>0:
					#time.sleep(1)
					#count()
					self.widget.after(1000, count)
				else:
					self.widget.config(text="Now you can sync your videos!!",fg = "black",font = ("Times",18))
...
canvas.pack() 
widget = Label(canvas,font=("Times",20),bg="white")
widget.pack()
canvas.create_window(165,25,window=widget)
img = Image.open("images/first.png")  # PIL solution
