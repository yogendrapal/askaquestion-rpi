...
class check_buttons(Thread):
	...
	def update1(self,ind):
		...
		frame = frames[ind]
		ind += 1
		self.canvas.itemconfig(self.canvas.create_image(0, 0, anchor="nw", image=frame),image=frame)
		'''if ind==2:
			time.sleep(1)
		if ind%21==0:
			time.sleep(1)'''

		if GPIO.input(16)!=0 and GPIO.input(18)!=0 and GPIO.input(12)!=0:
			root.after(100, self.update1, ind)
		else:
			frames=[]
